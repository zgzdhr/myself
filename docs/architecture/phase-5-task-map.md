...
- 不做完全自主 Agent 排程；
- 不在第一版做复杂多设备冲突合并。
- 不做拖拽排程、冲突检测、重复计划、外部日历导入和时间追踪；这些留给后续小阶段。

### Phase 5E：可控记忆管理增强

当前优先级：暂缓，先完成国内版发布前稳定化。
...
Phase 5D 第一版已经完成，但三星真机试用说明当前最重要的工作不是继续扩功能，
而是把现有闭环变成国内网络下可以稳定使用的版本。

当前建议顺序：

1. 完成首页、任务、提醒、同步等真机反馈修复，并持续用真实输入做回归测试。
2. 注册腾讯云或阿里云账号，优先购买便宜的国内轻量应用服务器。
3. 准备域名、备案和 HTTPS，把 `apps/api` 从 Vercel 复制部署到国内服务器。
4. 保留当前 Vercel + Supabase 配置作为海外版，不删除现有部署。
5. 国内版第一阶段只替换 `PARSER_BASE_URL`，先让 `/parse`、`/review`、`/plan`
   在中国大陆手机流量下可访问。
6. 国内版第二阶段再决定账号和数据库：
   - 短期可以继续使用 Supabase，验证国内网络可达性；
   - 如果登录和同步仍受网络影响，再迁移到国内 PostgreSQL / 云数据库和自建账号接口；
   - 不一次性重写全部账号、同步和数据库能力。
7. 国内版真实试用稳定后，再进入 Phase 5E 可控记忆管理增强。
